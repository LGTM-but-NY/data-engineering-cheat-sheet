...
\begin{multicols}{3}
    ...
    \subsubsection{Heuristics chọn Index (Q12, 13 - F1.1)}
    \begin{itemize}[leftmargin=*, nosep]
        \item \textbf{Equality (=):} Hash Index (tốt nhất) > B+-tree.
        \item \textbf{Range ($>, <$, BETWEEN):} B+-tree (bắt buộc) > Clustering (tốt) > Secondary. Hash vô dụng.
        \item \textbf{Update Heavy:} Index làm chậm thao tác ghi (Insert/Update/Delete) vì phải cập nhật cả Index tree.
        \item \textbf{Small File:} Linear Scan thường tốt hơn Index (do overhead I/O của index).
    \end{itemize}
    ...
    \begin{itemize}[leftmargin=*, label={--}]
        ...
        \item \textbf{Ngữ cảnh sử dụng:}
            ...
            \end{itemize}
        \item \textbf{Lưu ý khi trả lời trong đề thi:} Quan điểm đúng nhất thường là \textit{không} chọn tuyệt đối ETL hay ELT — cả hai đều có ưu, nhược điểm riêng và việc lựa chọn phụ thuộc vào ngữ cảnh ứng dụng cụ thể (yêu cầu hiệu năng, kích thước dữ liệu, khả năng xử lý của DW, chính sách bảo mật, v.v.).
    \end{itemize}
    ...
    \begin{itemize}[leftmargin=*, label={--}]
        \item \textbf{Extract (Trích xuất):}
        \begin{itemize}
            \item \textit{Filter:} Lọc data rác/cũ (VD: \texttt{WHERE is\_deleted = false}).
            \item \textit{Dimensions:} Dùng \texttt{SELECT DISTINCT} từ bảng transaction để lấy list duy nhất (Khách hàng, SP).
            \item \textit{Facts:} \texttt{JOIN} các bảng giao dịch (Order + OrderDetail) để lấy đủ thông tin.
        \end{itemize}
        \item \textbf{Transform (Biến đổi):}
        \begin{itemize}
            \item \textit{Calc Measures:} Tính cột phái sinh (VD: \texttt{Revenue = Price * Quantity}).
            \item \textit{Time Hierarchy:} Tách \texttt{Date} $\to$ Day, Month, Quarter, Year.
            \item \textit{Loc Hierarchy:} Tách Address $\to$ District, City, Region.
            \item \textit{Lookups:} Map ID từ nguồn sang Surrogate Key của DW.
        \end{itemize}
        \item \textbf{Load (Nạp):}
        \begin{itemize}
            \item \textit{Fact:} Nạp **Incremental** (chỉ nạp dòng mới).
            \item \textit{Dimension:} Nạp **Overwrite** (nếu nhỏ) hoặc **SCD/Upsert** (nếu lớn/cần lịch sử).
            \item \textit{Schedule:} Định thời chạy (Daily/Weekly).
        \end{itemize}
    \end{itemize}
