...
\begin{multicols}{3}
    ...
    \paragraph{Capacity Calculation Example}\mbox{}\\
    ...
    \textit{4 dạng bài chính \& cách trình bày}

    \subsection{Thiết kế Kho Dữ Liệu (Star Schema)}
    \begin{itemize}[leftmargin=*, label={--}]
        \item \textbf{Bước 1: Fact Table (Bảng Sự kiện)}
        \begin{itemize}
            \item \textit{Tên:} Phản ánh quy trình (VD: \texttt{Sales\_Fact}).
            \item \textit{PK:} Composite Key từ các FK (VD: \texttt{TimeID, StoreID, ProductID}).
            \item \textit{Measures:} Cột số liệu (Revenue, Qty). \textit{Lưu ý:} \texttt{Price} thường ở Dim, \texttt{Revenue} ở Fact.
        \end{itemize}
        \item \textbf{Bước 2: Dimension Tables (Bảng Chiều)}
        \begin{itemize}
            \item \textit{Time Dim:} Luôn có. Hierarchy: Day $\to$ Month $\to$ Quarter $\to$ Year.
            \item \textit{Other Dims:} Product, Customer, Store. Cấu trúc: \texttt{Surrogate Key} (PK), \texttt{Business Key} (Source ID), Attributes.
        \end{itemize}
        \item \textbf{Mẹo trình bày:}
        \newline $\rightarrow$ \textbf{Vẽ hình:} Star Schema (Fact ở giữa, Dims xung quanh).
        \newline $\rightarrow$ \textbf{Liệt kê Text:} Viết rõ schema dạng text để giám khảo dễ chấm.
        \newline \textit{VD:} \texttt{Dim\_Time(TimeID, Day, Month, Year)}
    \end{itemize}

    \subsection{Kiến trúc \& Công nghệ}
    \begin{itemize}[leftmargin=*, label={--}]
        \item \textbf{Kiến trúc:} Đề xuất mô hình \textbf{3-Tier} (Source $\to$ Staging $\to$ DW $\to$ Data Mart) hoặc \textbf{Lambda} (Batch + Speed) cho Big Data.
        \item \textbf{Integration Flow:}
        \begin{itemize}
            \item \textbf{ETL:} Nếu cần làm sạch kỹ, data nhạy cảm.
            \item \textbf{ELT:} Nếu đích là Cloud DW (BigQuery) hoặc Hadoop (tận dụng sức mạnh xử lý đích).
        \end{itemize}
        \item \textbf{Tech Stack (Phân loại rõ):}
        \begin{itemize}
            \item \textit{Ingestion:} Kafka (Streaming), Airflow (Orchestrator).
            \item \textit{Processing:} Spark (Processing), MapReduce.
            \item \textit{Storage:} HDFS / S3 (Data Lake), MongoDB (NoSQL).
            \item \textit{Serving/DW:} Hive, BigQuery, Snowflake.
        \end{itemize}
    \end{itemize}

    \subsection{SQL \& ETL Logic}
    \begin{itemize}[leftmargin=*, label={--}]
        \item \textbf{Extract:} Dùng \texttt{SELECT DISTINCT} cho Dimension.
        \item \textbf{Transform:} Dùng hàm time (\texttt{Month()}, \texttt{Year()}) tạo Time Dim.
        \item \textbf{SCD Type 2:} Nếu đề hỏi về "lưu lịch sử thay đổi" \newline $\rightarrow$ Thêm cột \texttt{Effective\_Date}, \texttt{Expiry\_Date}, \texttt{Current\_Flag}.
    \end{itemize}

    \subsection{Lý thuyết Big Data}
    \begin{itemize}[leftmargin=*, label={--}]
        \item \textbf{CAP vs ACID:} So sánh Consisteny (Replicas vs Constraints).
        \item \textbf{MapReduce Trace:} Vẽ bảng Input/Output từng pha: Map $\to$ Shuffle (Sort/Group) $\to$ Reduce.
    \end{itemize}
